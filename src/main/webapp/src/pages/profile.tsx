...
import {
    fetchProfile, fetchOutlookCalendars, fetchGoogleCalendars,
    connectOutlook, setOutlookCalendar, setGoogleCalendar,
        ...
        <Layout>
            <Stack spacing={4}>
                <Box>
                    ...
                    <Typography variant="body2" sx={{mt: 1}}>Manage your Microsoft and Google connections.</Typography>
                </Box>
                        ...
                        <CardContent>
                            {profile?.outlookConnected ? (
                                    ...
                                    {profile.outlookProfilePath && (
                                        <Box>
                                            <Typography variant="caption" color="text.secondary"
                                                        sx={{textTransform: 'uppercase', letterSpacing: 1}}>Profile
                                                File</Typography>
                                            <Typography variant="body2" sx={{
                                                fontFamily: 'monospace',
                                                mt: 0.5,
                                                wordBreak: 'break-word'
                                            }}>{profile.outlookProfilePath}</Typography>
                                        </Box>
                                    )}
                                    <FormControl fullWidth>
                                        <InputLabel>Select Source Calendar</InputLabel>
                                        <Select value={profile.outlookCalendarId || ''} label="Select Source Calendar"
                                                onChange={(e) => handleOutlookCalendarChange(e.target.value as string)}>
                                            {outlookCalendars.map(cal => <MenuItem key={cal.id}
                                                                                   value={cal.id}>{cal.name}</MenuItem>)}
                                        </Select>
                                    </FormControl>
                                    <Divider/>
                                    <Button variant="outlined" color="error" onClick={handleDisconnectOutlook}>
                                        Remove Profile File
                                    </Button>
                                </Stack>
                            ) : (
                                <Box sx={{
                                    ...
                                    <Typography variant="body2" sx={{mb: 3}}>Enter the path to your Outlook profile file
                                        (.ost or .pst) to connect.</Typography>
                                    <Stack spacing={2} sx={{maxWidth: 400, mx: 'auto'}}>
                                        ...
                                        />
                                        <Button variant="contained" startIcon={<MonitorIcon/>}
                                                onClick={handleConnectOutlook} disabled={!profilePath.trim()}>
                                            Connect Outlook
                                        </Button>
                                    </Stack>
                                </Box>
                            ...
                            {profile?.googleConnected ? (
                                <Stack spacing={3}>
                                    <FormControl fullWidth>
                                        ...
                                        <Select value={profile.googleCalendarId || ''}
                                                label="Select Destination Calendar"
                                                onChange={(e) => handleGoogleCalendarChange(e.target.value as string)}>
                                            {googleCalendars.map(cal => <MenuItem key={cal.id}
                                                                                  value={cal.id}>{cal.name}</MenuItem>)}
                                        </Select>
                                    </FormControl>
                                    <Divider/>
                                    <Stack direction="row" spacing={2}>
                                        <Button variant="outlined" onClick={handleGoogleLogin}>Reconnect Google Account</Button>
                                ...
                                <Box sx={{
                                    ...
                                    <Typography variant="body2" sx={{mb: 2}}>Connect your Google account to select a
                                        destination calendar.</Typography>
                                    <Button variant="contained" startIcon={<GoogleIcon/>} onClick={handleGoogleLogin}>Sign
