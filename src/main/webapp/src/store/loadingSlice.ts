...
const loadingSlice = createSlice({
  ...
  reducers: {
    requestStarted: (state) => { state.pending += 1; },
    requestFinished: (state) => { state.pending = Math.max(0, state.pending - 1); },
  },
});

export const { requestStarted, requestFinished } = loadingSlice.actions;
export const selectIsLoading = (state: { loading: { pending: number } }) => state.loading.pending > 0;
