...
const logsSlice = createSlice({
  ...
  reducers: {
    clearLogDetails: (state) => { state.details = null; },
  },
  extraReducers: (builder) => {
    builder
      .addCase(fetchLogs.fulfilled,       (state, { payload }) => { state.list = payload; })
      .addCase(fetchLogDetails.fulfilled, (state, { payload }) => { state.details = payload; });
  },
});
