...
export function setupInterceptors(dispatch: AppDispatch) {
  ...
  axiosInstance.interceptors.response.use(
    (response) => { if (!response.config.silent) dispatch(requestFinished()); return response; },
    (error)    => { if (!error.config?.silent)   dispatch(requestFinished()); return Promise.reject(error); },
  );
}
