...
const truncatePath = (path?: string, maxLength: number = 50): string => {
    ...
    const lastSlashIndex = path.lastIndexOf('/') > 0 ? path.lastIndexOf('/') : path.lastIndexOf('\\');
...
export const connectOutlook = createAsyncThunk('profile/connectOutlook', (body: OutlookConnection) =>
    outlookApi.connectOutlook(body).then(() => undefined));
...
export const setOutlookCalendar = createAsyncThunk('profile/setOutlookCalendar', (body: CalendarSelection) =>
    outlookApi.setOutlookCalendar(body).then(() => undefined));
...
export const setGoogleCalendar = createAsyncThunk('profile/setGoogleCalendar', (body: CalendarSelection) =>
    googleApi.setGoogleCalendar(body).then(() => undefined));
    ...
    extraReducers: (builder) => {
        builder
            .addCase(fetchProfile.fulfilled, (state, {payload}) => {
                const profile: Profile = {...payload, outlookProfilePath: truncatePath(payload.outlookProfilePath)};
